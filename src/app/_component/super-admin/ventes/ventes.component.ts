...
export class VentesComponent implements OnInit, AfterViewInit {
  ...
  constructor(private globalSrc: VenteService, private agentService: AgentService, private route: ActivatedRoute, private ngxService: NgxSpinnerService, private router: Router, private builder: FormBuilder, private bustripService: BustripService) {
  }
  ...
  subStatForm() {
    ...
    if (this.f.date.value !== '') {
      // console.log('Recherche par Date == ' + this.f.date.value);
      // this.date = new Date(this.f.date.value);
      this.currentStat = 'Day';
      // this.resetForm();
      this.tableValues = [];
      ...
      this.globalSrc.oneCompagnieVenteByDay(+hubId, dateInf, +this.currentAgent.id).subscribe(
        value => {
          if (value !== null) {
    ...
    if (this.f.month.value !== '' && this.f.month.value !== '0' && this.f.year.value !== '0' && this.f.year.value !== '') {
      ...
      this.tableValues = [];
      this.currentTableValue = [];
      this.month = VentesComponent.returnMonth(+this.f.month.value);
      this.year = this.f.year.value;
      console.log('Rechercher par Mois == ' + this.f.month.value + ' - ' + this.f.year.value);
      this.globalSrc.oneCompagnieVenteByMonth(this.selectedHub, this.currentAgent.id, this.f.year.value, this.f.month.value).subscribe(
        value => {
          if (value !== null) {
            this.allCompagnieVente = value;
            this.getDataValue(value);
    ...
    if (this.f.date1.value !== '' && this.f.date2.value !== '') {
      ...
      this.globalSrc.oneCompagnieVente(this.selectedHub, this.f.date1.value, this.f.date2.value, this.currentAgent.id).subscribe(
        value => {
          this.allCompagnieVente = value;
          this.getDataValue(value);
        }, err => console.log('Erreur donnée day == ' + err)
  ...
  getDataValue(allCompagnieVente) {
    ...
    for (const oneValue of allCompagnieVente) {
      ...
      for (const oneArriveLigne of  oneValue.lignesArrive) {
        ...
        const values = VentesComponent.getValueByOrders(reservationOfOneLigneOfOneArrive);
        ...
        const tauxRemplissageLigneAller = values[5];
      ...
      tauxRemplissageAller = (tauxRemplissageAller / compteur);
      ...
      for (const oneRetourLigne of  oneValue.lignesDepart) {
        ...
        const values = VentesComponent.getValueByOrders(reservationOfOneLigneOfOneRetour);
        // Pour les valeurs totales Aller
        ...
        const tauxRemplissageLigneRetour = values[5];
      ...
      tauxRemplissageRetour = (tauxRemplissageRetour / compteur);
      ...
      const values = VentesComponent.getLigneTotal(allerOfLigne, retourOfLigne);
